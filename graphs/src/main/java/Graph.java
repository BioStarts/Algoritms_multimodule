...
public class Graph {
    ...
    public void addEdge(String start, String finish){
        if ( !find(start) || !find(finish) ) {
            throw new IllegalArgumentException(String.format("Invalid label for vertex [%s, %s]",start,finish));
        }
    ...
    public void dfs(String startLabel){ //англ. Depth-first search, DFS
        if ( !find(startLabel) ){
    ...
    public void bfs(String startLabel){ //англ. breadth-first search, DFS
        if ( !find(startLabel) ){
            throw new IllegalArgumentException("Invalid startLabel " + startLabel);
        }
